...
const AddProposal = (props) => {
  ...
  const [enteredProposalURL, setEnteredProposalURL] = useState("");
  ...
  const inputValidate = (name, value, type) => {
    ...
    }else if (name === "proposalURL") {
      setIsValid({
        ...isValid,
        proposalURL: validate(value, type),
      });
    }
  };
  ...
  const onBlurHandler = (event) => {
    ...
    } else if (event.target.name === "proposalURL") {
      setIsTouched({
        ...isTouched,
        proposalURL: true,
      });
  ...
  const proposalURLHandler = (event) => {
    setEnteredProposalURL(event.target.value);
    inputValidate(event.target.name, event.target.value, [VALIDATOR_REQUIRE()]);
  };

  const formHandler = () => {
    const formData = {
      wsProposalTitle: enteredTitle,
      authorName: enteredAuthorName,
      authorEmail: enteredAuthorEmail,
      coverImgURL: enteredCoverImgURL,
      wsProposalLink: enteredProposalURL
    };
    console.log(formData);
    setLoading(true);
    if (isValid.title && isValid.authorName && isValid.authorEmail && isValid.coverImgURL && isValid.proposalURL) {
      EditorService.addWSProposal(formData).then((res) => {
        if (res) {
          setTimeout(() => {
            setLoading(false);
  ...
  return (
    ...
    >
      <DialogTitle id="form-dialog-title" style={{ textAlign: "center" }}>
        Publish Workshop Proposal
      </DialogTitle>
      <form>
        <DialogContent>
          <DialogContentText>
            Publish workshop proposal to front
          </DialogContentText>
          ...
          <TextField
            required
            error={!isValid.proposalURL && isTouched.proposalURL}
            name="proposalURL"
            margin="dense"
            id="proposalURL"
            label="Workshop Proposal URL"
            type="text"
            fullWidth
            onChange={proposalURLHandler}
            onBlur={onBlurHandler}
          />
          <br />
          <br />
